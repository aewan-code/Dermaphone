...
struct HelloPhotogrammetry{
    ...
    init(inputFolder: String, outputFilename : String ) {
   ...
   // public static let configuration = CommandConfiguration(
      //  abstract: "Reconstructs point cloud from a folder of images.")
    
  //  @Argument(help: "The local input file folder of images.")
  //  private var inputFolder: String = "/Users/ace20/Downloads/testskinlesion"
    
        //  @Argument(help: "Full path to the USDZ output file.")
   // private var outputFilename: String = "/Users/ace20/Documents/pointcloud"
  ...
  /*  @Option(name: [.customShort("o"), .long],
    ...
    private func makeRequestFromArguments() -> PhotogrammetrySession.Request {
        ...
        if let detailSetting = detail {
            return PhotogrammetrySession.Request.pointCloud
        } else {
            return PhotogrammetrySession.Request.pointCloud
        }
    }
...
private func savePointCloudToFile(_ pointCloud: PhotogrammetrySession.PointCloud) {
    let outputFilename = "/Users/ace20/Documents/pointcloud.txt"

    // Create a string representing the point cloud data
    var pointCloudData = ""
    for point in pointCloud.points {
        pointCloudData.append("\(point.position),\n")
    }

    // Write the point cloud data to a file
    do {
        try pointCloudData.write(toFile: outputFilename, atomically: true, encoding: .utf8)
        logger.log("Point cloud data saved to: \(outputFilename)")
    } catch {
        logger.error("Error saving point cloud data: \(error)")
    }
}
